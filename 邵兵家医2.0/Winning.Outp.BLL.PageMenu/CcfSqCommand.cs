...
{
    public class CcfSqCommand : ICommand
    {
        public RequestResult Execute(object sender, EventArgs e)
        {
            ...
            {
                object ErrMsg = null;
                ret = GlobalVariable.HisSys.Reflected.RunNetApp("Winning.Outp.UI.CcfSq.dll", "Winning.Outp.UI.CcfSq.StartUp", "Run", out ErrMsg, pat);
            }
            return new FrameWork.Core.Common.RequestResult { Success = ret };
        }

        public string ID { get { return MenuCommandId.CCFSQCommandID; } }
    }
}
